...
//
import UIKit
protocol FTTagsAlignment {}

/// Defines a horizontal alignment for UI elements.
public enum FTTagsHorizontalAlignment: FTTagsAlignment {
    case left
    case right
    case leading
    case trailing
...
public enum FTTagsVerticalAlignment: FTTagsAlignment {
    case top
    case center
    case bottom
}

/// A horizontal alignment used internally by `FTTagsAlignedCollectionViewFlowLayout`
/// to layout the items, after resolving layout direction specifics.
private enum EffectiveHorizontalAlignment: FTTagsAlignment {
    case left
    case right
    case justified
}
...
private struct AlignmentAxis<A: FTTagsAlignment> {
...
open class FTTagsAlignedCollectionViewFlowLayout: UICollectionViewFlowLayout {
    
    // MARK: - 🔶 Properties
    
    /// Determines how the cells are horizontally aligned in a row.
    /// - Note: The default is `.justified`.
    public var horizontalAlignment: FTTagsHorizontalAlignment = .justified
    
    /// Determines how the cells are vertically aligned in a row.
    /// - Note: The default is `.center`.
    public var verticalAlignment: FTTagsVerticalAlignment = .center
    ...
    fileprivate var effectiveHorizontalAlignment: EffectiveHorizontalAlignment {
        ...
        var trivialMapping: [FTTagsHorizontalAlignment: EffectiveHorizontalAlignment] {
            return [
                .left: .left,
                .right: .right,
                .justified: .justified
    ...
    fileprivate var alignmentAxis: AlignmentAxis<FTTagsHorizontalAlignment>? {
        switch effectiveHorizontalAlignment {
        case .left:
            return AlignmentAxis(alignment: FTTagsHorizontalAlignment.left, position: sectionInset.left)
        case .right:
            guard let collectionViewWidth = collectionView?.frame.size.width else {
                return nil
            }
            return AlignmentAxis(alignment: FTTagsHorizontalAlignment.right, position: collectionViewWidth - sectionInset.right)
        default:
            return nil
        }
    }
    ...
    ///                          (Default: `.center`)
    public init(horizontalAlignment: FTTagsHorizontalAlignment = .justified, verticalAlignment: FTTagsVerticalAlignment = .center) {
        super.init()
    ...
    private func verticalAlignmentAxisForLine(with layoutAttributes: [UICollectionViewLayoutAttributes]) -> AlignmentAxis<FTTagsVerticalAlignment>? {
    ...
    fileprivate func verticalAlignmentAxis(for currentLayoutAttributes: UICollectionViewLayoutAttributes) -> AlignmentAxis<FTTagsVerticalAlignment> {
...
fileprivate extension UICollectionViewLayoutAttributes {
    ...
    func isRepresentingFirstItemInLine(collectionViewLayout: FTTagsAlignedCollectionViewFlowLayout) -> Bool {
        if currentItem <= 0 {
            return true
        }
        else {
    ...
    func isRepresentingLastItemInLine(collectionViewLayout: FTTagsAlignedCollectionViewFlowLayout) -> Bool {
    ...
    func align(toAlignmentAxis alignmentAxis: AlignmentAxis<FTTagsHorizontalAlignment>) {
    ...
    func align(toAlignmentAxis alignmentAxis: AlignmentAxis<FTTagsVerticalAlignment>) {
        switch alignmentAxis.alignment {
        case .top:
    ...
    private func alignToPrecedingItem(collectionViewLayout: FTTagsAlignedCollectionViewFlowLayout) {
    ...
    ///
    /// - Parameter collectionViewLayout: The layout on which to perfom the calculations.
    private func alignToFollowingItem(collectionViewLayout: FTTagsAlignedCollectionViewFlowLayout) {
        let itemSpacing = collectionViewLayout.minimumInteritemSpacing
    ...
    func alignHorizontally(collectionViewLayout: FTTagsAlignedCollectionViewFlowLayout) {
    ...
    func alignVertically(collectionViewLayout: FTTagsAlignedCollectionViewFlowLayout) {
