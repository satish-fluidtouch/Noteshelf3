...
//  DailyNotes
//
//  Created by Siva Kumar Reddy on 23/09/20.
...
open class DataCache {
    static let cacheDirectoryPrefix = "com.ns3.Imagecache."
    static let ioQueuePrefix = "com.ns3.Imagequeue."
    static let defaultMaxCachePeriodInSecond: TimeInterval = 60 * 60 * 24 * 7         // a week

    public static var instance = DataCache(name: "ns3_Cache")
...
extension DataCache {
    ...
    public func write(dictionary: [AnyHashable: Any], forKey key: String) {
        write(object: dictionary as NSCoding, forKey: key)
    }

    /// Write an array for key
    public func write(array: [Any], forKey key: String) {
    ...
    public func readObject(forKey key: String) -> NSObject? {
        ...
        if let data = data, let unarchivedData = try? NSKeyedUnarchiver.unarchivedObject(ofClasses: [NSObject.self], from: data) {
            return  unarchivedData as? NSObject;
        }
        return nil
    }
...
extension DataCache {
    ...
    public func clean(byKey key: String, onCompletion: (() -> Void)?) {
