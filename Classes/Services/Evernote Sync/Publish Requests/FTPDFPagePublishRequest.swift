...
class FTPDFPagePublishRequest: FTBasePublishRequest {
    ...
    fileprivate func deleteResourceForPage(_ pageRecord: ENSyncRecord!) {
        ...
        pageRecord.noteStoreClient().fetchNote(withGuid: pageRecord.parent.enGUID, includingContent: true, resourceOptions: ENResourceFetchOption.includeAttributes) { (note, error) in
            if let error = error {
                self.executeBlock {
                        ...
                        if let pageRecord = try self.managedObjectContext()?.existingObject(with: self.objectID!) as? ENSyncRecord {
                            ...
                            if orderedResources.contains(where: {nil != pageRecord.enGUID && ($0 as AnyObject).guid == pageRecord.enGUID}) {
                                ...
                                if pageContents?.count == 1 {
                                    note?.resources = orderedResources.filter({($0 as AnyObject).guid != pageRecord.enGUID})
                                }
                                ...
                                let enml = FTENSyncUtilities.enmlRepresentation(withResources: note?.resources, syncRecords: pagesForENContent);
                                ...
                                note?.updated = NSNumber(value: (Date.init(timeIntervalSinceReferenceDate: lastUpdated) as NSDate).edamTimestamp );
                                ...
                                pageRecord.noteStoreClient().update(note!, completion: { (updatedNote, error) in
                                    if let error = error {
                                        self.executeBlock {
                                            FTENSyncUtilities.recordSyncLog(String(format: "Failed with Error:%@",error as CVarArg));
                                            self.delegate?.didCompletePublishRequestWithError!(error);
                        ...
                        if let pageRecord = try self.managedObjectContext()?.existingObject(with: self.objectID!) as? ENSyncRecord {
                            ...
                            self.getPage(fromPageRecord: pageRecord, onCompletion: { (pdfPage) in
                                if let pdfPage = pdfPage {
                                    ...
                                    if orderedResources.contains(where: {nil != pageRecord.enGUID && ($0 as AnyObject).guid == pageRecord.enGUID}) {
                                        if pageRecord.isContentDirty {
                                            ...
                                            if let resource = resource {
                                                ...
                                                contentSizeForFlurry = resource.data.size.int32Value;
                                            }
                                            else {
                                    ...
                                    let enml = FTENSyncUtilities.enmlRepresentation(withResources: resourcesMappedForAllPages, syncRecords: pagesForENContent);
                                    note?.content = String.init(format: EVERNOTE_NOTE_TEMPLATE, enml!);
                                    
                                    note?.resources = resourcesMappedForAllPages;
                                    ...
                                    if(pageRecord.isContentDirty)
                                    {
                                        ...
                                        note?.updated = NSNumber(value: (Date(timeIntervalSinceReferenceDate: pdfPage.lastUpdated.doubleValue) as NSDate).edamTimestamp as Int64);
                                    }
                                    else
                                    {
                                    ...
                                    if(pdfPage is FTPageTagsProtocol) {
                                        (pdfPage as! FTPageTagsProtocol).tags().forEach{note?.tagNames.append($0 as String)};
                                    }
                                    note?.tagNames?.forEach{tags.add($0 )};
                                    note?.tagNames = (tags.allObjects as! [String]);
                                    self.closeDocumentIfNeeded();
                                    ...
                                    pageRecord.noteStoreClient().update(note!, completion: { (updatedNote, error) in
                                        if let error = error {
                                            self.executeBlock(onPublishQueue: {
                                                ...
                                                if ((error as NSError).code == ENErrorCode.limitReached.rawValue)
                                                {
                                            ...
                                            self.executeBlock {
                                                do {
                                                    if let pageRecord = try self.managedObjectContext()?.existingObject(with: self.objectID!) as? ENSyncRecord {
                                                        ...
                                                        if let _ = resource {
                                                            if let edamResources = updatedNote?.resources {
