...
class FTDiaryFormatIpadPort: FTDairyFormat {
    ...
    override func renderYearPage(context: CGContext, months : [FTMonthInfo], calendarYear : FTYearFormatInfo) {
        ...
        let yearX = baseBoxX ;
        let yearY = baseBoxY - 16 - 185;
        
        let attrs : [NSAttributedString.Key : Any] =  [.font : UIFont.appFont(for: .light, with: screenInfo.fontsInfo.yearPageDetails.yearFontSize),
        ...
        months.forEach { (month) in
            ...
            yearRectsInfo.monthRects.append(getLinkRect(location: CGPoint(x: renderX, y: renderY),
                                                        frameSize: CGSize(width: monthCellWidth, height: monthCellHeight)))
            curMonthIndex += 1;
            if(curMonthIndex > 3) {
                curMonthIndex = 1;
                renderX = monthStartX;
    ...
    override func renderDayPage(context: CGContext, dayInfo: FTDayInfo) {
        ...
        var quoteAttrs:[NSAttributedString.Key:Any]=[NSAttributedString.Key.font:UIFont.appFont(for: .medium, with: 12),
    ...
    override func renderMonthTitle(monthInfo: FTMonthlyCalendarInfo) {
        ...
        let monthY:CGFloat = baseBoxY - 68 - monthAttr.size().height
        ...
        let yearY:CGFloat = monthY + monthAttr.size().height - yearAttr.size().height - 25
        ...
        monthAttr.append(yearAttr);
        monthAttr.draw(at: CGPoint.init(x:baseBoxX, y:  baseBoxY - 68 - monthAttr.size().height));
        
        let cellWidth = (pageRect.width - (2*baseBoxX - 1))/7
        
        let localAttrs : [NSAttributedString.Key : Any] =  [.font :UIFont.appFont(for: .medium, with: 19),
        ...
        var startX = baseBoxX + 10;
        let startY = baseBoxY - 9 - symbol.size().height;
    ...
    override  func render(dayInfo : [FTDayInfo], calendarYear: FTYearFormatInfo)
    {
        ...
        var attrs : [NSAttributedString.Key : Any] =  [.font :UIFont.appFont(for: .medium, with: 19),
                                                      NSAttributedString.Key.kern : 0.8];
        
        let startX = baseBoxX;
        let startY = baseBoxY;
        let cellWidth =  (pageRect.width - (2*baseBoxX))/7
        let cellHeigth = (pageRect.height - baseBoxY - boxBottomOffset)/6;
        
        let cellOffsetX: CGFloat = 10
        let cellOffsetY:CGFloat = 7
        ...
        dayInfo.forEach { (eachDay) in
            ...
            let attr = NSAttributedString.init(string: eachDay.dayString, attributes: attrs);
            attr.draw(in: frame)
