...
class FTOneDriveAlertHandler: NSObject {
    static func showOneDriveAuthenticationAlertIfNeeded(_ onController: UIViewController){
        ...
        if UserDefaults.oneDriveAuthAlertShown == false && oneDriveAlertController == nil {
            if let isOneDriveAutoBackup = FTCloudBackUpManager.shared()?.currentBackUpCloudType(), let isLoggedIn = FTCloudBackUpManager.shared()?.activeCloudBackUpManager.isLoggedIn() {
                if isOneDriveAutoBackup == .oneDrive && isLoggedIn == false {
