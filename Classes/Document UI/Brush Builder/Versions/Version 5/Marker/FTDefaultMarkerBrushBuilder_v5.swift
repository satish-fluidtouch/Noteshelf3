//
//  FTDefaultMarkerBrushBuilder_v5.swift
//  Noteshelf
//
...
final class FTDefaultMarkerBrushBuilder_v5: FTBrushProtocol {
    ...
    static func brushImageFor(brushWidth: CGFloat, scale: CGFloat) -> UIImage {
        return FTDefaultMarkerBrushBuilder_V4.brushImageFor(brushWidth: brushWidth, scale: scale)
    }

    static func thicknessCorrectionFactorFor(brushWidth: CGFloat) -> CGFloat {
        return FTDefaultMarkerBrushBuilder_V4.thicknessCorrectionFactorFor(brushWidth: brushWidth)
    }
}

//MARK: Zero Size
private extension FTDefaultMarkerBrushBuilder_v5 {
    ...
    static func _attributesFor(brushWidth: CGFloat, velocitySensitive: Bool) -> FTPenAttributes {
        let attributes = FTDefaultMarkerBrushBuilder_V4.attributesFor(brushWidth: brushWidth, velocitySensitive: velocitySensitive);
        return attributes;
    }
}
