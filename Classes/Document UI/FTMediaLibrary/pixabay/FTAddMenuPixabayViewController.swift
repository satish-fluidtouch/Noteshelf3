...
class FTAddMenuPixabayViewController: UIViewController, UIGestureRecognizerDelegate, FTPopoverPresentable {
    ...
    let viewModel = FTAddMenuPixabayViewModel()
    let cellIdentifier = "PixabayCell"
    ...
    override func viewDidLoad() {
        ...
        self.back.isHidden = toHideBackBtn
...
extension FTAddMenuPixabayViewController: FTSegmentedControlDelegate {
...
// MARK: - Search
extension FTAddMenuPixabayViewController {
...
//        self.searchBar.searchTextField.text = ""
        ...
        task = Task {
            await viewModel.fetchPixabay(pageNo: page)
        }
    }
}

// MARK: - Load More
extension FTAddMenuPixabayViewController: UICollectionViewDataSourcePrefetching {
...
extension FTAddMenuPixabayViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: cellIdentifier, for: indexPath) as? FTPixabayCollectionViewCell else {
...
extension FTAddMenuPixabayViewController: UICollectionViewDelegate {
...
extension FTAddMenuPixabayViewController {
    class func showAddMenuPixaBayController(from controller: UIViewController, source: Any, toHideBackBtn: Bool = false) -> FTAddMenuPixabayViewController {
        let storyboard = UIStoryboard.init(name: "FTDocumentEntity", bundle: nil)
        guard let pixabayVc = storyboard.instantiateViewController(withIdentifier: "FTAddMenuPixabayViewController") as? FTAddMenuPixabayViewController else {
            fatalError("Programmer error, FTAddMenuUnFTAddMenuPixabayViewControllersplashViewController not found")
        }

        pixabayVc.toHideBackBtn = toHideBackBtn
        pixabayVc.view.backgroundColor = UIColor.appColor(.popoverBgColor)
        pixabayVc.ftPresentationDelegate.source = source as AnyObject
        controller.ftPresentPopover(vcToPresent: pixabayVc, contentSize: AddMenuType.media.contentSize, hideNavBar: true)
        return pixabayVc
    }
}
