...
private extension FTPageViewController
{
    func copySelectedAnnotations() {
        ...
        do {
            ...
            dict["antsViewFrame"] = antsView.frame;
            ...
            if let data = pbItems.first?[UIPasteboard.pdfAnnotationUTI()] as? Data {
                do {
                    if let dict = try NSKeyedUnarchiver.unarchivedObject(ofClasses: [NSObject.self],
                        ...
                        let contentOffset = self.scrollView?.contentOffset ?? CGPoint.zero;

                        var antsViewNewFrame = antsViewFrame;
                        if let tappedPoint = point {
                            let convertedTappedPoint = CGPointTranslate(tappedPoint, -contentOffset.x, -contentOffset.y);
                            
                            antsViewNewFrame.origin.x = convertedTappedPoint.x - (antsViewFrame.width)*0.5;
                            antsViewNewFrame.origin.y = convertedTappedPoint.y - (antsViewFrame.height)*0.5;
...
{
    func lassoContentViewControllerDidEndEditing(with initialFrame: CGRect, currentFrame: CGRect, controller: FTLassoContentSelectionViewController) {
